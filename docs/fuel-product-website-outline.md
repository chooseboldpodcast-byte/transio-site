...
**Context:** The live site (`index.html`) currently markets one product, ShiftCheck, under a company-level "Physical AI for convenience retail" hero. This outlines how to add **Fuel Reconciliation** as product #2.
...
1. **The site is already built for it.** The hero is a *company-level* category claim; everything below is ShiftCheck. The umbrella exists — we just show it now holds two products.
...
3. **Two products, two wallets.** ShiftCheck sells **time** (5-minute close). Fuel sells **money** (you're paying for fuel you never received). Same motion — *perceive → reconcile → flag* — pointed at the delivery instead of the shelf.
...
**Success looks like:** a visitor understands in ~5s that Trasio has two products, self-selects by pain (time vs money), and the fuel buyer reaches a "show me what I overpaid" CTA. **Failure:** burying ShiftCheck, or the home reading like two unrelated companies.
...
- **(B) Platform home + per-product pages** *(recommended)* — `/shiftcheck` and `/fuelmatch`. Current deep ShiftCheck content moves to `/shiftcheck`; home keeps hero + a two-product split + top proof.
...
- **Unifying motion across both products: Perceive → Reconcile → Flag.** ShiftCheck perceives the shelf; Fuel perceives the bill of lading. Both reconcile against the system of record and flag the variance. Reuses the exact brand vocabulary (`RECONCILED` / `MATCHED` / `VARIANCE DETECTED`).
...
Also adopt: a **calm-confidence hero** — *"Know your fuel bill is right"* — paired with the loss proof beneath it (more on-brand than a pure loss-aversion headline); an explicit **ShiftCheck platform-fit** section; and **consultative CTAs** ("Talk to us about FuelMatch" / "Schedule a walkthrough").
...
- **Nav:** `Product` → **`Products ▾`** dropdown (ShiftCheck · FuelMatch). Keep `Technology · Pricing · Blog`. CTA stays "Join Founding Stores."
- **New "Two products" section** (after hero, before "Engineered for the Edge") — ready copy from the designer (price-variance language removed per the hidden-feature decision):
  ...
  - **Card 1 — `SHIFT MANAGEMENT` · Trasio ShiftCheck:** "Close shifts in 5 minutes. AI counts inventory behind the counter, timestamps every handoff, and flags disputes automatically. No barcode guns, no nightly recount loop." → "See ShiftCheck →" (`/shiftcheck`)
...
- **`/shiftcheck`** — existing deep content (Sarah/Mike scenarios, three benefits, Inside ShiftCheck) moves here largely as-is.
- **`/fuelmatch`** — new page (section outline in §3).
- **Pricing page** — FuelMatch shows **"Contact us"** for now (no published price), consistent with the consultative CTA; ShiftCheck pricing unchanged.
...
Mirror the ShiftCheck page *rhythm* (same family) but lead with money:
...
9. **Platform fit** — runs alongside ShiftCheck on the same tablets + login; ShiftCheck protects cash/inventory at handoff, FuelMatch protects fuel spend.
...
- **SEO:** new keyword cluster — *fuel delivery reconciliation, bill of lading reconciliation, fuel overbilling, c-store fuel margin, BOL audit.* Net-new surface area ShiftCheck owns none of.
...
Still open:
- **Build approach for the static site** (`index.html` is a single hand-built HTML file ~53KB; per-product pages = new `shiftcheck.html` / `fuelmatch.html` or a templating step — decide before implementation).
